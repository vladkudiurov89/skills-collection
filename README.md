...
Библиотека скиллов для Claude Code — сюда складываются интересные находки,
чтобы не потерять и иметь под рукой готовые к установке.
...
(RAG-память между сессиями, помечено как stub на момент публикации).
